...
export function IconCloud({ icons, images }: IconCloudProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (targetRot) {
        const elapsed = performance.now() - targetRot.startTime;
        const progress = Math.min(1, elapsed / targetRot.duration);
        const easedProgress = easeOutCubic(progress);

        rotationRef.current = {
          x:
            targetRot.startX +
            (targetRot.x - targetRot.startX) * easedProgress,
          y:
            targetRot.startY +
            (targetRot.y - targetRot.startY) * easedProgress,
        };
      ...
      iconPositions.forEach((icon, index) => {
        ...
        const scale = (rotatedZ + 560) / 840;
        const opacity = Math.max(0.2, Math.min(1, (rotatedZ + 420) / 560));
